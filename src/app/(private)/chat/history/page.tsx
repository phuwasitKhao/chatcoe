...
export default function ChatHistoryPage() {
  ...
  const fetchChats = async () => {
    try {
      ...
      const response = await fetch(`/api/v1/chat/chats?userId=${userId}&includeLatestMessage=true`);
            ...
            >
              <div className="flex items-center">
                ...
                <div>
                  ...
                  {chat.latestMessages && chat.latestMessages.length > 0 && (
                    <div className="text-sm text-gray-500 truncate">
                      {chat.latestMessages[0].content.substring(0, 50)}
                      {chat.latestMessages[0].content.length > 50 ? '...' : ''}
                    </div>
                  )}
